...
parser.add_argument('-m', required=True, metavar='topic', help='which topic')
...
for topic in sorted(qrels_dt):
    ...
    if topic == t:
        print_res = run_path.split('/')[-1].split('_')
        print print_res[0], '    ', print_res[1], '    ', print_res[2], '    ', print_res[3], '    ', print_res[4], '    ', topic_ndcg0
        result_0 += topic_ndcg0
        result_1 += topic_ndcg1
